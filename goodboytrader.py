...
startup_message = (
    ...
    f"⏰ Schedule: Checking every 15m candle close"
)
...
def monitor_points():
    while True:
        try:
            ...
            # Log monitor output instead of printing (Render won't display console)
            monitor_output = (
                f"📊 GoodBoyTrader - Real-Time Entry Points Monitor\n"
                f"⏰ Current Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
                f"💰 Current Price: {current_price}\n"
                f"⏳ Next Trade Check: {next_run}\n"
            ...
            print(monitor_output)  # Keep for local testing
...
schedule.every().hour.at(":15").do(run_trading_logic)
schedule.every().hour.at(":30").do(run_trading_logic)
schedule.every().hour.at(":45").do(run_trading_logic)

# --- Main Loop ---
position_state, trade = load_trade_state()
msg = "Starting scheduler and monitor... Waiting for next 15m candle close."
print(msg)
logging.info(msg)
